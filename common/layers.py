...
class Sigmoid:
    ...
    def backward(self, dout):
        return dout * sigmoid_grad(self.out)
...
class SoftmaxWithLoss:
    ...
    def backward(self, dout = 1):
        ...
        if self.t.size == self.y.size:#监督数据为one-hot形式
...
class Dropout:
    ...
    def forward(self, x, train_flag = True):
        if train_flag:
            self.mask = np.random.randn(*x.shape) > self.dropout_rate
            return x * self.mask
        else:
...
class Pooling:
    ...
    def backward(self, dout):
        ...
        dmax[np.arange(self.max_arg.size), self.max_arg.flatten()] = dout.flatten()
